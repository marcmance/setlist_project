    ...
    else
    {
        if (isset($_FILES['photo']))
        {
            ...
            if (!isset($msg)) // If there was no error
            {
				...
				//UPDATE `setlist_db`.`album` SET `ext`='.jpg' ;


                mysql_query("UPDATE album
                                SET ext='$ext' cover_art='$data' WHERE `id`='1'");
...
elseif (isset($_GET['show']))
{
    ...
    list($ext, $cover_art) = mysql_fetch_row($result);
...
<?php
$result = mysql_query("SELECT * from album ORDER BY id DESC");
...
else
{
    ...
    while(list($id, $name) = mysql_fetch_row($result))
    {
        ...
        echo "<a href='{$PHP_SELF}?show={$id}'>{$name}</a> &ndash; ";
    }
